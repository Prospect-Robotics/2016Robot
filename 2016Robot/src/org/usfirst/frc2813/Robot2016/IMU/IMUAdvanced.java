...
public class IMUAdvanced extends IMU {
	...
	protected int decodePacketHandler(byte[] received_data, int offset,
		...
		int packet_length = IMUProtocol.decodeQuaternionUpdate(received_data,
				offset, bytes_remaining, quaternion_update_data);
		if (packet_length > 0) {
	...
	private void updateWorldLinearAccelHistory(float x, float y, float z) {
		...
		world_linear_accel_history[next_world_linear_accel_history_index] = Math
				.abs(x) + Math.abs(y);
	...
	public float getAverageFromWorldLinearAccelHistory() {
		...
		return world_linear_accel_history_sum
				/ WORLD_LINEAR_ACCEL_HISTORY_LENGTH;
	}
	...
	private void setQuaternion(IMUProtocol.QuaternionUpdate raw_update) {
		synchronized (this) {
			...
			ypr[0] = (float) Math.atan2(2 * q[1] * q[2] - 2 * q[0] * q[3], 2
					* q[0] * q[0] + 2 * q[1] * q[1] - 1);
			ypr[1] = (float) Math.atan(gravity[0]
					/ Math.sqrt(gravity[1] * gravity[1] + gravity[2]
							* gravity[2]));
			ypr[2] = (float) Math.atan(gravity[1]
					/ Math.sqrt(gravity[0] * gravity[0] + gravity[2]
							* gravity[2]));
			...
			q_product[0] = q[0] * q2[0] - q[1] * q2[1] - q[2] * q2[2] - q[3]
					* q2[3]; // new w
			q_product[1] = q[0] * q2[1] + q[1] * q2[0] + q[2] * q2[3] - q[3]
					* q2[2]; // new x
			q_product[2] = q[0] * q2[2] - q[1] * q2[3] + q[2] * q2[0] + q[3]
					* q2[1]; // new y
			q_product[3] = q[0] * q2[3] + q[1] * q2[2] - q[2] * q2[1] + q[3]
					* q2[0]; // new z
			...
			q_final[0] = q_product[0] * q_conjugate[0] - q_product[1]
					* q_conjugate[1] - q_product[2] * q_conjugate[2]
					- q_product[3] * q_conjugate[3]; // new w
			q_final[1] = q_product[0] * q_conjugate[1] + q_product[1]
					* q_conjugate[0] + q_product[2] * q_conjugate[3]
					- q_product[3] * q_conjugate[2]; // new x
			q_final[2] = q_product[0] * q_conjugate[2] - q_product[1]
					* q_conjugate[3] + q_product[2] * q_conjugate[0]
					+ q_product[3] * q_conjugate[1]; // new y
			q_final[3] = q_product[0] * q_conjugate[3] + q_product[1]
					* q_conjugate[2] - q_product[2] * q_conjugate[1]
					+ q_product[3] * q_conjugate[0]; // new z

			world_linear_acceleration_x = q_final[1];
			world_linear_acceleration_y = q_final[2];
			world_linear_acceleration_z = q_final[3];

			updateWorldLinearAccelHistory(world_linear_acceleration_x,
					world_linear_acceleration_y, world_linear_acceleration_z);
			...
			float MAG_X = raw_update.mag_x * cos_pitch + raw_update.mag_z
					* sin_pitch;
			float MAG_Y = raw_update.mag_x * sin_roll * sin_pitch
					+ raw_update.mag_y * cos_roll - raw_update.mag_z * sin_roll
					* cos_pitch;
			float tilt_compensated_heading_radians = (float) Math.atan2(MAG_Y,
					MAG_X);
